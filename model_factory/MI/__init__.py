...
def extract_bags(bags, Y):

  feature = [item['instance'][0] for item in bags if item['label'][0]==Y]
...
def cross_validation(data, k):
  ...
  for t in range(k):
    vals = data[n*t:n*(t+1)]
    trains = data[:n*t] + data[n*(t+1):]
    yield trains, vals


def print_evaluation_result(clf, bags_test, args):
  pred_score = np.array([clf(np.array(B['instance'])) for B in bags_test])
  pred_label = np.array([1 if score >= 0 else -1 for score in pred_score])
  true_label = np.concatenate([B['label'] for B in bags_test])
  ...
  auc = metrics.roc_auc_score(true_label, pred_score)
#
#   if not args.aucplot:
...
from model_factory.MI import PU
