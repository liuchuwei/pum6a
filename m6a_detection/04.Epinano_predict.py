...
ap.add_argument("-k", "--kernel",
                help="kernel used for training SVM, choose any one from 'linear', 'poly', 'rbf', 'sigmoid'; if no choice made, all 4 kernels will be used")
...
ap.add_argument('-mc', '--modification_status_column',
                help="column number from (input file1, i.e, traing file) that contains modification status information")

requiredGrp = ap.add_argument_group('required arguments')
# requiredGrp.add_argument("-t","--train", required=True, help="file name of feature table used for training")
requiredGrp.add_argument("-p", "--predict", required=True,
                         help="file name of feature table used for making predictions or testing accuracy; can be gzipped.\nwhen this file is the same the one used for training, half of the data will be chosen for training.")
requiredGrp.add_argument('-cl', '--columns', required=True,
                         help="comma separated column number(s) that contain features used for training and prediction")
...
if 'predict' in args:
    ...
    predict_df = pd.read_csv(args['predict'], compression='gzip') if args['predict'].endswith('.gz') else pd.read_csv(
        args['predict'])
...
if args['out_prefix']:
    ...
    out_prefix = m_u_var.split('/')[-1].split('.')[0] + '.' + args['predict'].split('/')[-1].split('.')[
        0] + '.' + '.'.join(names) + '.MODEL'
...
if args['model']:
    ...
    if args['predict'] == args['train']:
        X_train, X_test, y_train, y_test, indices_train, indices_test = train_test_split(X, Y.values.ravel(), indices,
                                                                                         test_size=0.5,
                                                                                         random_state=100)
    else:
        X_train, _, y_train, _, indices_train, _ = train_test_split(X, Y.values.ravel(), indices, test_size=0,
                                                                    random_state=100)
        X_test = predict_tmp.iloc[:, cols]
        indices_test = X_test.index.tolist()
...
if args['model']:
    for _, m in enumerate(args['model'].strip().split(',')):
        ...
        for t in range(len(indices_test)):
            idx = indices_test[t]
            original_line = ",".join(map(str, df_tmp.iloc[idx].values)) if args['predict'] == args[
                'train'] else ",".join(map(str, predict_tmp.iloc[idx].values))
